...
export function CompanyWorkspace({ tab, token, companyName, onNavigate, onCountsChange }: { tab: string; token: string; companyName: string; onNavigate: (tab: string) => void; onCountsChange: (counts: CompanyCounts) => void }) {
  ...
  const verified = profile?.emailVerified && profile.verificationStatus === 'VERIFIED';
  ...
  const openModal = (kind: Exclude<ModalKind, null>, item?: Station | Charger | Employee) => {
    ...
    if (kind === 'verification') setForm({ gstNumber: profile?.gstNumber ?? '', kycDocumentUrl: profile?.kycDocumentUrl ?? '', businessAddress: profile?.businessAddress ?? '' });
    setModal(kind);
    setError('');
  };
  ...
  const updateChargerStatus = async (charger: Charger, status: Charger['status']) => {
    try {
      await apiRequest(`/company/chargers/${charger.id}`, { method: 'PUT', ...auth, body: JSON.stringify({ stationId: charger.stationId, chargerCode: charger.chargerCode, connectorType: charger.connectorType, powerKw: charger.powerKw, status, maintenanceMode: status === 'MAINTENANCE', firmwareVersion: charger.firmwareVersion, healthScore: charger.healthScore }) });
      setNotice(`${charger.chargerCode} is now ${status.toLowerCase()}.`); await loadAll();
    } catch (statusError) { setError(statusError instanceof Error ? statusError.message : 'Unable to update charger.'); }
  };
  ...
  const askAi = async () => {
    try {
      const response = await apiRequest<{ answer: string }>('/company/ai/ask', { method: 'POST', ...auth, body: JSON.stringify({ question }) });
  ...
  return (
    <section className="company-workspace">
      ...
      {!verified && <div className="company-verification-banner"><ShieldCheck size={20} /><div><strong>Finish company verification</strong><span>{!profile?.emailVerified ? 'Verify your company email, then submit GST/KYC for admin approval.' : `Business verification: ${profile?.verificationStatus ?? 'PENDING'}`}</span></div><button onClick={() => !profile?.emailVerified ? setEmailVerificationOpen(true) : openModal('verification')}>{!profile?.emailVerified ? 'Verify email' : 'Submit documents'}</button></div>}
      ...
      {tab === 'dashboard' && <Dashboard dashboard={dashboard} onNavigate={onNavigate} />}
      ...
      {tab === 'revenue' && <RevenuePanel dashboard={dashboard} analytics={analytics} onDownload={download} />}
      {tab === 'maintenance' && <MaintenancePanel chargers={chargers} alerts={dashboard.alerts} onStatus={updateChargerStatus} />}
      ...
      {tab === 'ai' && <AiPanel question={question} setQuestion={setQuestion} answer={assistantAnswer} onAsk={askAi} dashboard={dashboard} />}
      {tab === 'reports' && <ReportsPanel onDownload={download} />}
      {tab === 'notifications' && <NotificationsPanel items={notifications} />}
      {tab === 'settings' && <SettingsPanel profile={profile} saving={saving} onProfile={() => openModal('profile')} onVerification={() => openModal('verification')} onVerifyEmail={() => setEmailVerificationOpen(true)} onSettings={updateCompanySettings} />}

      {modal && <CompanyModal kind={modal} form={form} setForm={setForm} stations={stations} editing={Boolean(editingId)} saving={saving} onClose={() => setModal(null)} onSubmit={submit} />}
      {emailVerificationOpen && <EmailVerificationDialog code={verificationCode} sent={verificationSent} saving={saving} onCode={setVerificationCode} onSend={sendVerificationCode} onConfirm={confirmVerificationCode} onInbox={() => { setEmailVerificationOpen(false); onNavigate('notifications'); }} onClose={() => setEmailVerificationOpen(false)} />}
...
function AiPanel({ question, setQuestion, answer, onAsk, dashboard }: { question: string; setQuestion: (value: string) => void; answer: string; onAsk: () => void; dashboard: CompanyDashboard }) { return <div className="ai-company-layout"><article className="company-card ai-company-card"><div className="ai-orb"><Bot size={30} /></div><div><div className="feature-eyebrow">VIDYUT AI</div><h2>Ask your network</h2><p>Demand, maintenance, pricing, energy, revenue and expansion insights use your company-scoped data.</p></div><div className="ai-answer"><Sparkline /><strong>{answer}</strong></div><div className="ai-input"><input value={question} onChange={event => setQuestion(event.target.value)} onKeyDown={event => { if (event.key === 'Enter') void onAsk(); }} placeholder="Ask about today's revenue…" /><button onClick={() => void onAsk()}><Send size={17} /></button></div><div className="ai-prompts">{["Show today's revenue",'Which chargers need maintenance?','When is peak demand?','Where should we expand?'].map(item => <button key={item} onClick={() => setQuestion(item)}>{item}</button>)}</div></article><aside className="company-card"><div className="company-card-head"><div><h2>Live context</h2><p>Used for every answer</p></div><Activity size={20} /></div><div className="insight-list"><SmallMetric label="Stations" value={dashboard.totalStations} /><SmallMetric label="Faults" value={dashboard.faults} /><SmallMetric label="Utilization" value={`${dashboard.utilizationRate}%`} /><SmallMetric label="Revenue" value={money(dashboard.revenue)} /></div></aside></div> }
