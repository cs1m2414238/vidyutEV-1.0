...
function AuditView({items,search}:{items:Audit[];search:string}) { const filtered=items.filter(i=>JSON.stringify(i).toLowerCase().includes(search.toLowerCase()));return <Panel title="Immutable admin activity" subtitle="Who changed what, previous and new values, timestamp and reason" icon={FileCheck2}><DataTable headers={['Time','Admin','Action','Resource','Previous → new','Reason']} rows={filtered.map(i=>[date(i.createdAt),`#${i.adminAccountId}`,i.action.replaceAll('_',' '),`${i.resourceType} ${i.resourceId||''}`,i.previousValue||i.newValue?`${i.previousValue||'—'} → ${i.newValue||'—'}`:'Recorded',i.reason||i.summary])}/></Panel> }
